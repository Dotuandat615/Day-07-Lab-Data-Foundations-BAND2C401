...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        # Base case: text fits within chunk size — return as-is
        ...
        # Character-level split (last resort when sep == "")
        if sep == "":
            return [
                current_text[i : i + self.chunk_size]
                for i in range(0, len(current_text), self.chunk_size)
        ...
        # Greedy merge: accumulate parts into buffer; recurse on oversized pieces
        result: list[str] = []
        current_buffer = ""
